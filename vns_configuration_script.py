...
# @copyright: This is a community script, anyone can use it under their own risk.
# @version: 1.0.1
# 
...
#     7) Creates Infrastructure NSG-BR Template
#     8) Creates Organization Profile for the Enterprise
#     9) Enable VSS flow and event collection in the Infrastructure system config
#     10) Creates New Enterprise
#     11) Creates New Enterprise user: admin, and add it to the enterprise admin group
#     12) Creates Two Domain Templates: one encrypted and one not encrypted
#     13) Creates Branch (encrypted) Domain, 3 Zones, and 5 Subnets
#     14) Creates DC (non encrypted) Domain, 3 Zones, and 3 Subnets
#     15) Creates NSG instances from the NSG templates
#     16) Creates NSG-BR instance from the NSG-BR templates, and assign it to Enterprise
#
...
nsgTemplate4Name = 'NSGV_2WAN_1LAN'
...
dczone1Name = "DC_Openstack"
dczone1Description = "Mountain View"
dczone2Name = "DC_Containers"
dczone2Description = "Mountain View"
dczone3Name = "DC_Physical"
dczone3Description = "Mountain View"
dcsubnet1Name = "RDO"
...
# license
myLicense = """MDEyOCckSkNO7ueXvPUeiuNQ//ui0vE9otuo4AnLJkLKuxoIJmVjIKzxXlMEqsA
...
LCJwcm9kdWN0VmVyc2lvCiI6IjQuMCIsImxpY2Vuc2VJZCI6MSwibWFqb3JSZWxlYXNlIjoxLCJtaW
...

...
######################################################
# Create NSG-V template (2 network, 1 access) 
######################################################
def func_create_nsg_2w_1l_br_template(csprootSession,nsgTemplateName,
                                infrastructure_gw_profile,infra_access_profile,
                                infrastructure_vsc_profile1,infrastructure_vsc_profile2):
    nsgateway_nsg = csprootSession.ns_gateway_templates.get_first(filter="name == '%s'" % nsgTemplateName)
    if not nsgateway_nsg:
        ...
        nsgateway_nsg = vspk.NUNSGatewayTemplate(name=nsgTemplateName, 
            description='Two Uplinks NSG-V', infrastructure_profile_id=infrastructure_gw_profile.id, 
            infrastructure_access_profile_id=infra_access_profile.id, personality='NSGBR', ssh_service='ENABLED', instance_ssh_override='ALLOWED')
        csprootSession.create_child(nsgateway_nsg)
...
###########################################
# Creating NSG-BR instance                #
###########################################
def func_create_nsgbr(csprootSession,nsgTemplate,NSGName,new_enterprise):
    new_nsg = csprootSession.ns_gateways.get_first(filter="name == '%s'" % NSGName)
    if not new_nsg:
        print 'INFO: Creating A New NSG instance', NSGName, 'and Assigning it to Enterprise'
        new_nsg=vspk.NUNSGateway(name=NSGName,template_id=nsgTemplate.id,entityScope="GLOBAL")
        csprootSession.create_child(new_nsg)
        #csprootSession.instantiate_child(new_nsg,nsgTemplate)    
        new_entpermission=vspk.NUEnterprisePermission(permitted_action="USE",permitted_entity_id=new_enterprise.id)
        new_nsg4.create_child(new_entpermission)
    else:
        print 'INFO: NSG instance', NSGName, 'is already created'
    return new_nsg
...
############################################# 
# Attach a vport to each subnet
############################################# 
def func_attach_vport(new_enterprise,vportname,nsg,portname,subnet,br_intfname):
    newvport = subnet.vports.get_first(filter="name == '%s'" % vportname)
    if not newvport:
        print 'INFO: Attaching vport', nsg.name, ":", portname, 'to a subnet'
        port = nsg.ns_ports.get_first(filter='name == "%s"' % portname)
        vlan = port.vlans.get_first(filter='value == 0')
        vlan_id = vlan.id
        newvport = vspk.NUVPort(name='%s' % vportname,address_spoofing="ENABLED",type="BRIDGE",vlanid='%s' % vlan_id)
        subnet.create_child(newvport)
        newbridge = vspk.NUBridgeInterface(name=br_intfname,vport_id=newvport.id)
        newvport.create_child(newbridge)
    else:
        print 'INFO: vPORT', nsg.name, ":", portname, 'is already attached'
...
def main():
    ...
    ######################################################
    # create 4 NSG templates (1WAN_1LAN, 2WAN_2LAN, 2WAN_4LAN, 2WAN_1LAN)
    ######################################################
    ...
    nsgtemplate_2w_1l = func_create_nsg_2w_1l_br_template(csprootSession,nsgTemplate4Name,infrastructure_gw_profile,
        infra_access_profile,infrastructure_vsc_profile1,infrastructure_vsc_profile2)
    ...
    new_dc_zone2 = func_create_zone(csprootSession,new_dc_domain,dczone2Name,dczone2Description)
    new_dc_zone3 = func_create_zone(csprootSession,new_dc_domain,dczone3Name,dczone3Description)
    # create a new subnets
    ...
    dc_subnet2_zone2 = func_create_subnet(csprootSession,new_dc_zone2,dcsubnet2Name,dczone2Description,dcsubnet2IpAddr,
                                           dcsubnet2Prefix,dcsubnet2DhcpMinRange,dcsubnet2DhcpMaxRange)
    dc_subnet3_zone1 = func_create_subnet(csprootSession,new_dc_zone3,dcsubnet3Name,dczone3Description,dcsubnet3IpAddr,
                                           dcsubnet3Prefix,dcsubnet3DhcpMinRange,dcsubnet3DhcpMaxRange)
    dc_subnet4_zone1 = func_create_subnet(csprootSession,new_dc_zone3,dcsubnet4Name,dczone3Description,dcsubnet4IpAddr,
                                           dcsubnet4Prefix,dcsubnet4DhcpMinRange,dcsubnet4DhcpMaxRange)

    ############################################# 
    # create 3 New NSGs Instances; one from each template, create NSG for csproot
    ############################################# 
    new_nsg1 = func_create_nsg(new_enterprise,nsgtemplate_2w_2l,NSGName1)
    new_nsg2 = func_create_nsg(new_enterprise,nsgtemplate_1w_1l,NSGName2)    
    new_nsg3 = func_create_nsg(new_enterprise,nsgtemplate_1w_1l,NSGName3)
    
    ############################################# 
    # create NSGBR Instance and assign it to enterprise
    ############################################# 
    #NSG BR
    new_nsg4 = func_create_nsgbr(csprootSession,nsgtemplate_2w_1l,NSGName4,new_enterprise)
   
    ############################################# 
    # Attach a vport to each subnet
    ############################################# 
    vportname = "nsg-b1-port3"
    portname = "port3"
    br_intfname = "ubuntu-vm1"
    newvport = func_attach_vport(new_enterprise,vportname,new_nsg1,portname,subnet1_zone1,br_intfname)

    vportname = "nsg-b1-port4"
    portname = "port4"
    br_intfname = "centos-vm3"
    newvport = func_attach_vport(new_enterprise,vportname,new_nsg1,portname,subnet2_zone1,br_intfname)
    
    vportname = "nsg-b2-port2"
    portname = "port2"
    br_intfname = "ubuntu-vm2"
    newvport = func_attach_vport(new_enterprise,vportname,new_nsg2,portname,subnet1_zone2,br_intfname)
 
    vportname = "nsg-hq-port2"
    portname = "port2"
    br_intfname = "firewall"
    newvport = func_attach_vport(new_enterprise,vportname,new_nsg3,portname,subnet1_zone3,br_intfname)
